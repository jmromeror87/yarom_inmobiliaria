<?php /** Libro Mayor — saldo inicial + movimiento del período + saldo final por cuenta */ ?>
...
<div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:.75rem;padding:12px 16px;font-size:12.5px;color:#1e40af;margin-bottom:16px;">
    💡 El Libro Mayor muestra, cuenta por cuenta, el saldo con que arrancó el período, lo que se movió (débito/crédito) durante el rango de fechas seleccionado, y el saldo con que cierra.
</div>
...
<table style="width:100%;border-collapse:collapse">
    <thead><tr style="background:{{ $info['bg'] }};border-bottom:2px solid {{ $info['border'] }}">
        <th style="text-align:left;padding:6px 10px;font-size:9.5px;color:{{ $info['color'] }};font-weight:700;width:10%">Código</th>
        <th style="text-align:left;padding:6px 10px;font-size:9.5px;color:{{ $info['color'] }};font-weight:700;width:30%">Cuenta</th>
        <th style="text-align:right;padding:6px 10px;font-size:9.5px;color:{{ $info['color'] }};font-weight:700;width:15%">Saldo Inicial</th>
        <th style="text-align:right;padding:6px 10px;font-size:9.5px;color:{{ $info['color'] }};font-weight:700;width:15%">Débito</th>
        <th style="text-align:right;padding:6px 10px;font-size:9.5px;color:{{ $info['color'] }};font-weight:700;width:15%">Crédito</th>
        <th style="text-align:right;padding:6px 10px;font-size:9.5px;color:{{ $info['color'] }};font-weight:700;width:15%">Saldo Final</th>
    </tr></thead>
    <tbody>
        ...
        <tr style="border-bottom:1px solid #F1F5F9">
            ...
            <td style="text-align:right;padding:7px 10px;font-family:monospace;font-size:11px;color:#64748b">{{ $fmt($r['saldo_inicial']) }}</td>
            ...
            <td style="text-align:right;padding:7px 10px;font-family:monospace;font-size:11px;color:#0F172A;font-weight:700">{{ $fmt($r['saldo_final']) }}</td>
        </tr>
        @endforeach
    </tbody>
</table>
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-top:18px;padding:16px 18px;border-radius:10px;border:2px solid {{ $cuadra ? '#BBF7D0' : '#FECDD3' }};background:{{ $cuadra ? '#F0FDF4' : '#FFF1F2' }}">
    <div>
        <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#64748B;margin-bottom:8px">Verificación de cuadre — movimiento del período</div>
        <div style="display:flex;gap:28px">
            <span style="font-size:13px;color:#0F172A">Mov. débitos: <strong style="font-family:monospace;color:#1E3A8A">{{ $fmt($data['total_debitos']) }}</strong></span>
            <span style="font-size:13px;color:#0F172A">Mov. créditos: <strong style="font-family:monospace;color:#7F1D1D">{{ $fmt($data['total_creditos']) }}</strong></span>
